...
const useApi = () => {
  ...
  useEffect(() => {
    ...
    if (currentGame.type === "GAME_RESULT") {
        const didGameEnded = _.find(startingGames, (g) => g.gameId === currentGame.gameId);
        if (didGameEnded) setStartingGames(current => _.filter(current, (g) => g.gameId !== currentGame.gameId))
  ...
  useEffect(() => {
       hResultsClient.get(REST_API).then((res) => {
        ...
        for (let i in endingGames) {
            const gameExists = _.find(newUpdatedData, (g) => g.gameId === endingGames[i].gameId);
            if (gameExists) setEndingGames(current => _.filter(current, (g) => g.gameId !== gameExists.gameId));
        }
     }) 
